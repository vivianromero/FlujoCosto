...
@register.simple_tag
def crud_nurl(obj, action, namespace=None):
    try:
        nurl = utils.crud_url_name(type(obj), action)
        if namespace:
            nurl = namespace + ':' + nurl
    ...
    try:
        ...
        if action in ['delete', 'update', 'detail']:
            if hasattr(obj, 'pk'):
                ...
                url = reverse(nurl, kwargs={'model_id': obj, 'pk': inline.pk})
        else:
...
@register.filter
def clean_query_search(getparams=None):
    """
    Clean the query variable filter from getparams
    """
    if getparams:
        query_count = getparams.find('query=')+6
        query_str = getparams[query_count:]
        rest_count = query_count + query_str.find('&')
        query_delete = getparams[query_count:rest_count]
        getparams = getparams.replace(query_delete, '')
        return getparams
    else:
        return '?'
