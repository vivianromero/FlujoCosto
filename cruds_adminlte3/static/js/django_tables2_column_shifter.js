...
$.django_tables2_column_shifter_init = function() {
    ...
    var load_states = function($table_class_container) {
        ...
        if (id in storage) {
            ...
            $('.' + button_shift_class_container).find(".btn-shift-column").each(function(){
                var $btn = $(this);
                var td_class = $btn.data("td-class");
...
// Add variable 'button_shift_class_container' to the API containing the class name to be returned
let button_shift_class_container = window.data.button_shift_class_container
...
$.django_tables2_column_shifter_hidden = function(idx) {
	...
	return $('.' + button_shift_class_container).eq(idx).find('.btn-shift-column').filter(function(z) {
