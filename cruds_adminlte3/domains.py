...
def eval_domain(self, domain: list):
    for i in range(domain.__len__()):
        if isinstance(domain[i], tuple):
            domain[i] = domain[i][0], eval(domain[i][1])
    return domain

def prefix_to_infix(self, domain: list):
    result = []
    dom = eval_domain(self, domain=domain)
    for token in range(dom.__len__() - 1, -1, -1):
        if dom[token] not in DOMAIN_OPERATORS:
            result.append(dom[token])
        else:
            if dom[token] == NOT_OPERATOR:
                s1 = result.pop()
                if isinstance(s1, Q):
                    q = ~s1
                else:
                ...
                s1 = result.pop()
                s2 = result.pop()
                if dom[token] == AND_OPERATOR:
                    q = Q(s1) & Q(s2)
                elif dom[token] == OR_OPERATOR:
                    q = Q(s1) | Q(s2)
                result.append(q)
    return result[0]
