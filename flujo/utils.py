...
@transaction.atomic
def actualiza_numeros(ueb, departamento, control, consecutivo):
    dicc_filter_consec = {'tiponumero': TipoNumeroDoc.NUMERO_CONSECUTIVO, 'ueb': ueb}
    dicc_filter_control = {'tiponumero': TipoNumeroDoc.NUMERO_CONTROL, 'ueb': ueb}
    ...
    if config[TipoNumeroDoc.NUMERO_CONSECUTIVO]['departamento']:
        dicc_filter_consec['departamento'] = departamento
        departamento_consec = departamento

    if config[TipoNumeroDoc.NUMERO_CONTROL]['departamento']:
        dicc_filter_control['departamento'] = departamento
        departamento_control = departamento

    consecutivos = NumeroDocumentos.objects.select_for_update().filter(**dicc_filter_consec)
    nros_consec_max = 0
    for p in consecutivos:
        nros_consec_max = p.numero if p.numero > nros_consec_max else nros_consec_max

    controles = NumeroDocumentos.objects.select_for_update().filter(**dicc_filter_control)
    nros_control_max = 0
    for p in controles:
        nros_control_max = p.numero if p.numero > nros_control_max else nros_control_max

    consecutivo_ = consecutivo
    control_ = control
    if nros_consec_max:
        consecutivo_ = nros_consec_max
        consecutivo_ = consecutivo_ if consecutivo_ > consecutivo else consecutivo
    if nros_control_max:
        control_ = nros_control_max
        control_ = control_ if control_ > control else control

    actualiza_nros = [NumeroDocumentos(ueb=ueb, numero=consecutivo_, tiponumero=TipoNumeroDoc.NUMERO_CONSECUTIVO,
                                       departamento=departamento_consec),
                      NumeroDocumentos(ueb=ueb, numero=control_, tiponumero=TipoNumeroDoc.NUMERO_CONTROL,
                                       departamento=departamento_control)
                      ]
    NumeroDocumentos.objects.bulk_update_or_create(actualiza_nros,
                                                   ['ueb', 'numero', 'tiponumero',
                                                    'departamento'],
...
@transaction.atomic
def actualiza_numero_eliminado(ueb, departamento, control, consecutivo):
    dicc_filter_consec = {}
    dicc_filter_control = {}
    ...
    documentos = Documento.objects.select_for_update().filter(ueb=ueb)

    if config[TipoNumeroDoc.NUMERO_CONSECUTIVO]['departamento']:
        dicc_filter_consec['departamento'] = departamento

    docs_consec = documentos.filter(**dicc_filter_consec)

    if config[TipoNumeroDoc.NUMERO_CONTROL]['departamento']:
        dicc_filter_control['departamento'] = departamento

    docs_control = documentos.filter(**dicc_filter_control)
    ...
    for p in docs_consec:
        max_consec = p.numeroconsecutivo if p.numeroconsecutivo > max_consec else max_consec
    ...
    NumeroDocumentos.objects.select_for_update().filter(**dicc_filter_consec).update(numero=max_consec)
    NumeroDocumentos.objects.select_for_update().filter(**dicc_filter_control).update(numero=max_control)
...
def existencia_anterior(doc, producto, elimina):
    ...
    ueb = doc.ueb
    # existencia_product = float(existencia) - float((cantidad*operacion))
